...
export default class Bag extends Component {
  ...
  convertNumToSplitString = (str: string) =>
  ...
  static updateBagCount() {
    document
      .querySelector('.header-bag-items-count')
      ?.classList.remove('two-num');
    document
      .querySelector('.header-bag-items-count')
      ?.classList.remove('two-num-plus');
    ...
    const bagCount = document.querySelector(
  ...
  createBagHeader() {
    ...
    const bagHeaderSubTitle = this.elFactory('p', {
      class: 'bag-header-subtitle',
    });
    bagHeaderSubTitle.textContent = 'Free delivery and free returns.';
    const continueShoppingLink = this.elFactory('a', {
      class: 'bag-btn-continue-shopping-link',
      href: './#main',
    });
    continueShoppingLink.textContent = 'Continue Shopping';
    const continueShoppingBtn = this.elFactory(
      'button',
      { class: 'bag-btn bag-btn-continue-shopping' },
  ...
  createBadTotal() {
    ...
    const bagTotalSummary = this.elFactory('div', {
      class: 'bag-total-summary',
    });
    const bagTotalSummaryTitle = this.elFactory('span', {
      class: 'bag-total-summaty-title',
    });
    bagTotalSummaryTitle.textContent = 'Total';

    const bagTotalSummaryValue = this.elFactory('span', {
      class: 'bag-total-summaty-value',
    });
    bagTotalSummaryValue.textContent = this.convertNumToSplitString(
      this.totalPrice,
    );
    const bagTotalSummaryValueCurrancy = this.elFactory('span', {
      class: 'bag-total-summaty-value-currancy',
    });
    bagTotalSummaryValueCurrancy.textContent = '$';

    const bagTotalSummaryValueWrapper = this.elFactory(
      'div',
      { class: 'bag-total-summaty-value-wrapper' },
      bagTotalSummaryValueCurrancy,
      bagTotalSummaryValue,
    );

    bagTotalSummary.append(bagTotalSummaryTitle);
    bagTotalSummary.append(bagTotalSummaryValueWrapper);

    const checkOutBtn = this.elFactory('button', {
      class: 'bag-btn bag-btn-check-out',
    });
  ...
  createBagItem(itemId: number) {
    ...
    const bagItemDiv = this.elFactory('div', {
      class: 'bag-item',
      'good-id': currentDBItem.id,
    });
    const bagItemImgDiv = this.elFactory('div', { class: 'bag-item-img' });
    const bagItemImg = this.elFactory('img', {
      class: 'img',
      src: currentDBItem.imgs[0],
      alt: 'bag-item-img',
    });
    bagItemImg.ondragstart = () => false;
    bagItemImgDiv.append(bagItemImg);

    const bagItemName = this.elFactory('a', {
      class: 'bag-item-name',
      href: `./#product&id=${currentDBItem.id}`,
    });
    bagItemName.textContent = this.setGoodsItemName(currentDBItem);
    const bagItemNameWrapper = this.elFactory(
      'div',
      { class: 'bag-item-name-wrapper' },
      bagItemName,
    );

    const bagItemQuantity = this.elFactory('div', {
      class: 'bag-item-quantity',
    });

    const bagItemQuantityMinusImg = this.elFactory('img', {
      class: 'bag-item-quantity-btn-img',
      src: './assets/images/icons/bag-item-quantity-minus.svg',
      alt: 'bag-item-quantity-minus-img',
    });
    bagItemQuantityMinusImg.ondragstart = () => false;
    const bagItemQuantityMinus = this.elFactory(
      'button',
      { class: 'bag-item-quantity-btn bag-item-quantity-minus' },
      bagItemQuantityMinusImg,
    );

    const bagItemQuantityPlusImg = this.elFactory('img', {
      class: 'bag-item-quantity-btn-img',
      src: './assets/images/icons/bag-item-quantity-plus.svg',
      alt: 'bag-item-quantity-plus-img',
    });
    bagItemQuantityPlusImg.ondragstart = () => false;
    const bagItemQuantityPlus = this.elFactory(
      'button',
      { class: 'bag-item-quantity-btn bag-item-quantity-plus' },
      bagItemQuantityPlusImg,
    );

    const bagItemQuantityInput = this.elFactory('input', {
      class: 'bag-item-quantity-input',
      type: 'number',
      min: '1',
      oninput: `validity.valid||(value='')`,
    });
    bagItemQuantityInput.value = `${
      Bag.bagItems.filter(el => +el.id === itemId)[0].count
    }`;

    const updateItemPrice = () => {
      const curPrice =
        +this.convertStringWithCommasToDefault(currentDBItem.price) *
        +bagItemQuantityInput.value;
      bagItemPriceValue.textContent = this.convertNumToSplitString(
        `${curPrice}`,
      );
    };
    ...
    const updateTotalPrice = () => {
      const allPriseVal = [
        ...document.querySelectorAll('.bag-item-price-value'),
      ]
    ...
    bagItemQuantityInput.addEventListener('input', () => {
      ...
      if (+inputVal >= 0 && inputVal !== '') {
        Bag.bagItems.map(x =>
          x.id === itemId ? (x.count = +inputVal) : false,
        );
        updateAllInfo();
      }
    });
    bagItemQuantityInput.addEventListener('blur', () => {
      if (bagItemQuantityInput.value === '') {
        ...
        Bag.bagItems.map(x =>
          x.id === itemId ? (x.count = +bagItemQuantityInput.value) : false,
        );
        updateAllInfo();
      }
    });
    ...
    const bagItemPriceDetails = this.elFactory('div', {
      class: 'bag-item-price-details',
    });

    const bagItemPriceValue = this.elFactory('span', {
      class: 'bag-item-price-value',
    });
    bagItemPriceValue.textContent = this.convertNumToSplitString(
      `${
        +this.convertStringWithCommasToDefault(currentDBItem.price) *
    ...
    const bagItemPriceCurrancy = this.elFactory('span', {
      class: 'bag-item-price-value-currancy',
    });
    bagItemPriceCurrancy.textContent = '$';

    const bagItemPriceValueWrapper = this.elFactory(
      'div',
      { class: 'bag-item-price-value-wrapper' },
      bagItemPriceCurrancy,
      bagItemPriceValue,
    );

    const bagItemRemoveBtn = this.elFactory('button', {
      class: 'bag-item-remove-btn',
    });
    ...
    bagItemRemoveBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        [...document.querySelectorAll('.bag-item')].map(el =>
          el.classList.add(moveHeightClass),
        );
      ...
      setTimeout(() => {
        ...
        [...document.querySelectorAll('.bag-item')].map(el =>
          el.classList.remove(moveHeightClass),
        );
      ...
      Bag.bagItems.map(el =>
        el.id === +itemId!
          ? Bag.bagItems.splice(Bag.bagItems.indexOf(el), 1)
          : false,
      );
    ...
    this.totalPrice = `${
      +this.totalPrice +
      +this.convertStringWithCommasToDefault(
        this.convertStringWithCommasToDefault(bagItemPriceValue.textContent),
      )
    }`;
  ...
  createCheckoutWindow() {
    ...
    const checkAllFields = () => {
      const allFields = [
        ...document.querySelectorAll('.checkout-info-field-input'),
      ] as HTMLInputElement[];
      const isAllFilled =
        allFields.filter(el => el.value !== '').length === allFields.length;
      isAllFilled && formValidation
        ? document
            .querySelector('.checkout-submit-btn')
    ...
    const checkoutHeaderCloseBtn = this.elFactory('button', {
      class: 'checkout-header-close-btn',
    });
    const checkoutHeaderCloseBtnImg = this.elFactory('img', {
      class: 'checkout-header-close-btn-img',
      src: './assets/images/icons/checkout-close-btn.svg',
      alt: 'checkout-header-close-btn-img',
    });
    ...
    const checkoutHeaderTitle = this.elFactory('h2', {
      class: 'checkout-header-title',
    });
    checkoutHeaderTitle.textContent = 'Checkout';

    const checkoutHeaderTotal = this.elFactory('div', {
      class: 'checkout-header-total',
    });
    checkoutHeaderTotal.textContent = 'Order Summary: $';
    const checkoutHeaderTotalSpan = this.elFactory('span', {
      class: 'checkout-header-total-value',
    });
    checkoutHeaderTotalSpan.textContent = `${this.convertNumToSplitString(
      this.totalPrice,
    )}`;
    ...
    const renderErrorImg = () => {
      const renderErrorImg = this.elFactory('img', {
        class: 'checkout-info-field-error-img',
        src: './assets/images/icons/error.svg',
        alt: 'checkout-info-field-error-img',
      });
    ...
    const checkoutInfoTitle = this.elFactory('div', {
      class: 'checkout-info-title',
    });
    checkoutInfoTitle.textContent = 'Where should we send your order?';

    // name adress block
    const checkoutInfoNameAddress = this.elFactory('div', {
      class: 'checkout-info-item',
    });

    const checkoutInfoNameAddressTitle = this.elFactory('p', {
      class: 'checkout-info-item-title',
    });
    ...
    // First name
    const checkoutInfoFirstName = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoFirstNameInput = this.elFactory('input', {
      class: 'checkout-info-field-input first-name-input',
      type: 'text',
      title: 'Please fill out this field.',
      autocomplete: 'given-name',
      maxlength: '14',
    });
    const checkoutInfoFirstNameTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoFirstNameTitle.textContent = 'First Name';

    const checkoutInfoFirstNameError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoFirstNameErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoFirstNameErrorText.textContent =
      'Please enter a valid first name (>2 characters).';

    checkoutInfoFirstNameError.append(
      renderErrorImg(),
      checkoutInfoFirstNameErrorText,
    );
    ...
    checkoutInfoFirstNameInput.addEventListener('focusout', () => {
      if (checkoutInfoFirstNameInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoFirstNameTitle,
          checkoutInfoFirstNameInput,
          checkoutInfoFirstNameError,
        );
      }
      if (
        checkoutInfoFirstNameInput.value.length < 3 &&
        checkoutInfoFirstNameInput.value.length !== 0
      ) {
        addErrorClass(
          checkoutInfoFirstNameTitle,
          checkoutInfoFirstNameInput,
    ...
    checkoutInfoFirstNameInput.addEventListener('input', () => {
      ...
      checkoutInfoFirstNameInput.value.length !== 0
        ? checkoutInfoFirstNameInput.removeAttribute('title')
        : checkoutInfoFirstNameInput.setAttribute(
            'title',
    ...
    // Last name
    const checkoutInfoLastName = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoLastNameInput = this.elFactory('input', {
      class: 'checkout-info-field-input last-name-input',
      type: 'text',
      title: 'Please fill out this field.',
      autocomplete: 'family-name',
      maxlength: '20',
    });
    const checkoutInfoLastNameTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoLastNameTitle.textContent = 'Last Name';

    const checkoutInfoLastNameError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoLastNameErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoLastNameErrorText.textContent =
      'Please enter a valid last name (>2 characters).';
    checkoutInfoLastNameError.append(
      renderErrorImg(),
      checkoutInfoLastNameErrorText,
    );
    ...
    checkoutInfoLastNameInput.addEventListener('focusout', () => {
      if (checkoutInfoLastNameInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoLastNameTitle,
          checkoutInfoLastNameInput,
          checkoutInfoLastNameError,
        );
      }
      if (
        checkoutInfoLastNameInput.value.length < 3 &&
        checkoutInfoLastNameInput.value.length !== 0
      ) {
        addErrorClass(
          checkoutInfoLastNameTitle,
          checkoutInfoLastNameInput,
    ...
    checkoutInfoLastNameInput.addEventListener('input', () => {
      ...
      checkoutInfoLastNameInput.value.length !== 0
        ? checkoutInfoLastNameInput.removeAttribute('title')
        : checkoutInfoLastNameInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    // Address
    const checkoutInfoAddress = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoAddressInput = this.elFactory('input', {
      class: 'checkout-info-field-input address-input',
      type: 'text',
      title: 'Please fill out this field.',
    });
    const checkoutInfoAddressTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoAddressTitle.textContent = 'Address';

    const checkoutInfoAddressError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoAddressErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoAddressErrorText.textContent =
      'Please enter a valid address (>2 words, each >4 characters).';
    checkoutInfoAddressError.append(
      renderErrorImg(),
      checkoutInfoAddressErrorText,
    );
    ...
    checkoutInfoAddressInput.addEventListener('focusout', () => {
      if (checkoutInfoAddressInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoAddressTitle,
          checkoutInfoAddressInput,
          checkoutInfoAddressError,
        );
      }
      const inputArr = checkoutInfoAddressInput.value.trim().split(' ');
      if (
        checkoutInfoAddressInput.value.length !== 0 &&
        (inputArr.length < 3 ||
    ...
    checkoutInfoAddressInput.addEventListener('input', () => {
      ...
      checkoutInfoAddressInput.value.length !== 0
        ? checkoutInfoAddressInput.removeAttribute('title')
        : checkoutInfoAddressInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    checkoutInfoAddress.append(
      checkoutInfoAddressInput,
      checkoutInfoAddressTitle,
      checkoutInfoAddressError,
    );

    checkoutInfoNameAddress.append(
      checkoutInfoFirstName,
      checkoutInfoLastName,
      checkoutInfoAddress,
    );

    // Contact info block
    const checkoutInfoContacts = this.elFactory('div', {
      class: 'checkout-info-item',
    });

    const checkoutInfoContactsTitle = this.elFactory('p', {
      class: 'checkout-info-item-title',
    });
    ...
    // Email address
    const checkoutInfoEmailAddress = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoEmailAddressInput = this.elFactory('input', {
      class: 'checkout-info-field-input email-input',
      type: 'email',
      title: 'Please fill out this field.',
      autocomplete: 'email',
      maxlength: '50',
    });
    const checkoutInfoEmailAddressTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoEmailAddressTitle.textContent = 'Email Address';

    const checkoutInfoEmailAddressError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoEmailAddressErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoEmailAddressErrorText.textContent =
      'Please enter a valid email address.';
    checkoutInfoEmailAddressError.append(
      renderErrorImg(),
      checkoutInfoEmailAddressErrorText,
    );
    ...
    checkoutInfoEmailAddressInput.addEventListener('focusout', () => {
      if (checkoutInfoEmailAddressInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoEmailAddressTitle,
          checkoutInfoEmailAddressInput,
          checkoutInfoEmailAddressError,
        );
      }
      if (
        checkoutInfoEmailAddressInput.value.length !== 0 &&
    ...
    checkoutInfoEmailAddressInput.addEventListener('input', () => {
      ...
      checkoutInfoEmailAddressInput.value.length !== 0
        ? checkoutInfoEmailAddressInput.removeAttribute('title')
        : checkoutInfoEmailAddressInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    checkoutInfoEmailAddress.append(
      checkoutInfoEmailAddressInput,
      checkoutInfoEmailAddressTitle,
      checkoutInfoEmailAddressError,
    );
    ...
    // Phone
    const checkoutInfoPhoneNum = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoPhoneNumInput = this.elFactory('input', {
      class: 'checkout-info-field-input phone-input',
      type: 'tel',
      title: 'Please fill out this field.',
      autocomplete: 'tel',
    });
    const checkoutInfoPhoneNumTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoPhoneNumTitle.textContent = 'Phone Number';

    const checkoutInfoPhoneNumError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoPhoneNumErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoPhoneNumErrorText.textContent =
      'Please enter a valid phone number.';
    checkoutInfoPhoneNumError.append(
      renderErrorImg(),
      checkoutInfoPhoneNumErrorText,
    );
    ...
    checkoutInfoPhoneNumInput.addEventListener('focusout', () => {
      if (checkoutInfoPhoneNumInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoPhoneNumTitle,
          checkoutInfoPhoneNumInput,
          checkoutInfoPhoneNumError,
        );
      }
      if (
        checkoutInfoPhoneNumInput.value.length !== 0 &&
        !validPhone.test(checkoutInfoPhoneNumInput.value)
      ) {
        addErrorClass(
          checkoutInfoPhoneNumTitle,
          checkoutInfoPhoneNumInput,
    ...
    checkoutInfoPhoneNumInput.addEventListener('input', () => {
      var newValue = checkoutInfoPhoneNumInput.value.replace(
        new RegExp(/[^\d\+]/, 'ig'),
        '',
      );
      ...
      checkoutInfoPhoneNumInput.value.length !== 0
        ? checkoutInfoPhoneNumInput.removeAttribute('title')
        : checkoutInfoPhoneNumInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    // Payment info title
    const checkoutInfoTitle2 = this.elFactory('div', {
      class: 'checkout-info-title',
    });
    checkoutInfoTitle2.textContent = 'How do you want to pay?';

    // Card info block
    const checkoutInfoCard = this.elFactory('div', {
      class: 'checkout-info-item',
    });

    const checkoutInfoCardTitle = this.elFactory('p', {
      class: 'checkout-info-item-title',
    });
    ...
    // Card
    const checkoutInfoCardNum = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoCardNumInput = this.elFactory('input', {
      class: 'checkout-info-field-input card-input',
      type: 'text',
      maxlength: '19',
      title: 'Please fill out this field.',
    });
    const checkoutInfoCardNumTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoCardNumTitle.textContent = `Credit/Debit Card Number`;

    const checkoutInfoCardNumError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoCardNumErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoCardNumErrorText.textContent =
      'Please enter a valid card number.';
    checkoutInfoCardNumError.append(
      renderErrorImg(),
      checkoutInfoCardNumErrorText,
    );

    const checkoutCardsImgs = this.elFactory('div', {
      class: 'checkout-info-field-cards',
    });
    const checkoutCardVisa = this.elFactory('img', {
      class: 'checkout-info-field-card visa',
      src: './assets/images/cards/visa.png',
      alt: 'checkout-info-field-card-img',
    });
    checkoutCardVisa.ondragstart = () => false;
    const checkoutCardMastercard = this.elFactory('img', {
      class: 'checkout-info-field-card mastercard',
      src: './assets/images/cards/mastercard.png',
      alt: 'checkout-info-field-card-img',
    });
    checkoutCardMastercard.ondragstart = () => false;
    const checkoutCardDiscover = this.elFactory('img', {
      class: 'checkout-info-field-card discover',
      src: './assets/images/cards/discover.png',
      alt: 'checkout-info-field-card-img',
    });
    checkoutCardDiscover.ondragstart = () => false;
    checkoutCardsImgs.append(
      checkoutCardVisa,
      checkoutCardMastercard,
      checkoutCardDiscover,
    );
    ...
    const reformatInputField = (inputEl: HTMLInputElement, target: string) => {
      ...
      const count = (text: string, target: string) => {
        let res = 0;
        if (target === 'card-num') {
          res = countSpaces(text);
        }
        if (target === 'card-exp') {
          res = countBackSpaces(text);
        }
        return res;
      };
    ...
    checkoutInfoCardNumInput.addEventListener('focusout', () => {
      if (checkoutInfoCardNumInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoCardNumTitle,
          checkoutInfoCardNumInput,
          checkoutInfoCardNumError,
        );
      }
      if (
        checkoutInfoCardNumInput.value.length !== 0 &&
        (checkoutInfoCardNumInput.value.length !== 19 ||
    ...
    checkoutInfoCardNumInput.addEventListener('input', () => {
      ...
      if (checkoutInfoCardNumInput.value.length >= 4) {
        if (cardValidation(checkoutInfoCardNumInput.value).success) {
          [...document.querySelectorAll('.checkout-info-field-card')].map(
            el => {
              el.classList.contains(
                `${cardValidation(checkoutInfoCardNumInput.value).name}`,
              )
      ...
      checkoutInfoCardNumInput.value.length !== 0
        ? checkoutInfoCardNumInput.removeAttribute('title')
        : checkoutInfoCardNumInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    const checkoutInfoExpirationDateCVVWrapper = this.elFactory('div', {
      class: 'checkout-info-wrapper',
    });

    // Expiration date
    const checkoutInfoExpirationDate = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoExpirationDateInput = this.elFactory('input', {
      class: 'checkout-info-field-input card-exp-input',
      type: 'text',
      maxlength: '5',
      title: 'Please fill out this field.',
    });
    const checkoutInfoExpirationDateTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoExpirationDateTitle.textContent = 'Expiration MM/YY';

    const checkoutInfoExpirationDateError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoExpirationDateErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoExpirationDateErrorText.textContent =
      'Please enter a valid expiration date.';
    checkoutInfoExpirationDateError.append(
      renderErrorImg(),
      checkoutInfoExpirationDateErrorText,
    );
    ...
    checkoutInfoExpirationDateInput.addEventListener('focusout', () => {
      if (checkoutInfoExpirationDateInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoExpirationDateTitle,
          checkoutInfoExpirationDateInput,
          checkoutInfoExpirationDateError,
        );
      }
      if (
        checkoutInfoExpirationDateInput.value.length !== 0 &&
    ...
    checkoutInfoExpirationDateInput.addEventListener('input', () => {
      ...
      checkoutInfoExpirationDateInput.value.length !== 0
        ? checkoutInfoExpirationDateInput.removeAttribute('title')
        : checkoutInfoExpirationDateInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    checkoutInfoExpirationDate.append(
      checkoutInfoExpirationDateInput,
      checkoutInfoExpirationDateTitle,
      checkoutInfoExpirationDateError,
    );
    ...
    // CVV
    const checkoutInfoCVV = this.elFactory('div', {
      class: 'checkout-info-field',
    });
    const checkoutInfoCVVInput = this.elFactory('input', {
      class: 'checkout-info-field-input card-cvv-input',
      type: 'text',
      maxlength: '3',
      title: 'Please fill out this field.',
    });
    const checkoutInfoCVVTitle = this.elFactory('span', {
      class: 'checkout-info-field-input-name',
    });
    checkoutInfoCVVTitle.textContent = 'CVV';

    const checkoutInfoCVVError = this.elFactory('div', {
      class: 'checkout-info-field-error',
    });
    const checkoutInfoCVVErrorText = this.elFactory('span', {
      class: 'checkout-info-field-error-msg',
    });
    checkoutInfoCVVErrorText.textContent =
    ...
    checkoutInfoCVVInput.addEventListener('focusout', () => {
      if (checkoutInfoCVVInput.value.length === 0) {
        ...
        addErrorClass(
          checkoutInfoCVVTitle,
          checkoutInfoCVVInput,
          checkoutInfoCVVError,
        );
      }
      if (
        checkoutInfoCVVInput.value.length !== 0 &&
        checkoutInfoCVVInput.value.length !== 3
      ) {
        addErrorClass(
          checkoutInfoCVVTitle,
          checkoutInfoCVVInput,
    ...
    checkoutInfoCVVInput.addEventListener('input', () => {
      checkoutInfoCVVInput.value = checkoutInfoCVVInput.value.replace(
        /[^\dA-Z]/g,
        '',
      );
      ...
      checkoutInfoCVVInput.value.length !== 0
        ? checkoutInfoCVVInput.removeAttribute('title')
        : checkoutInfoCVVInput.setAttribute(
            'title',
            'Please fill out this field.',
          );
      formValidation = true;
    ...
    checkoutInfoCVV.append(
      checkoutInfoCVVInput,
      checkoutInfoCVVTitle,
      checkoutInfoCVVError,
    );

    checkoutInfoExpirationDateCVVWrapper.append(
      checkoutInfoExpirationDate,
      checkoutInfoCVV,
    );

    checkoutInfoCard.append(
      checkoutInfoCardNum,
      checkoutInfoExpirationDateCVVWrapper,
    );
    ...
    const checkoutSubmit = this.elFactory('button', {
    ...
    checkoutSubmit.addEventListener('click', () => {
      ...
      document
        .querySelector('.header-bag-items-count')
        ?.classList.remove('active', 'two-num', 'two-num-plus');
      document
        .querySelector('.header-bag-img')
        ?.classList.remove('active', 'two-num', 'two-num-plus');
      document.querySelector('.bag-total')!.classList.add('hide');
      const bagTitle = document.querySelector(
        '.bag-header-title',
  ...
  createSuccessPurchaseMsg() {
    const successPurchase = this.elFactory('div', {
      class: 'success-purchase',
    });
  ...
  render() {
    ...
    document
      .querySelector('.header-menu-btn-block-wrapper')
