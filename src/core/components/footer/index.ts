...
export default class Footer extends Component {
  ...
  createBreadcrumbsPath() {
    const breadcrumbsPath = this.elFactory('div', {
      class: 'footer-breadcrumbs-path',
    });

    const onlineStoreItem = this.elFactory('div', {
      class: 'footer-breadcrumbs-path-el',
    });
    ...
    const onlineStoreItemsBrand = this.elFactory('div', {
      class: 'footer-breadcrumbs-path-el-items-brand',
    });
    const onlineStoreItemsCategory = this.elFactory('div', {
      class: 'footer-breadcrumbs-path-el-items-category',
    });
    const onlineStoreItemsCurrentItem = this.elFactory('div', {
      class: 'footer-breadcrumbs-path-el-items-current',
    });
    const onlineStoreItemsBag = this.elFactory('div', {
      class: 'footer-breadcrumbs-path-add-item',
    });
  ...
  createFooterLegal() {
    ...
    footerLegalCopyright.textContent =
      'Copyright © 2023 Online Store. All rights reserved.';
    ...
    const logoGithubTwoLink = this.elFactory('a', {
      class: 'footer-legal-logo',
      href: 'https://github.com/kornienkokostia',
      target: '_blanket',
    });
    const logoGithubTwoImg = this.elFactory('img', {
      class: 'footer-legal-img footer-legal-img-github',
      src: './assets/images/icons/github-logo.svg',
      alt: 'bag',
    });
