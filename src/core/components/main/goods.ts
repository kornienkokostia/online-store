...
export default class Goods extends Component {
  ...
  renderItems(
    ...
    const items = paginatedData.map(item => {
      ...
      const imgItem = this.elFactory('img', {
        class: 'img',
        src: item.imgs[0],
      });
      ...
      const imgDivWrapper = this.elFactory(
        'a',
        { class: 'goods-item-img-wrapper', href: `./#product&id=${item.id}` },
        imgDiv,
      );
      ...
      const goodInBag = this.elFactory(
        'div',
        {
      ...
      // rating
      const ratingStars = this.elFactory('div', {
        class: 'goods-item-wrapper-rating-stars',
      });
      for (let i = 0; i < 5; i++) {
        if (i < item.rating) {
          const ratingImg = this.elFactory('img', {
            class: 'goods-item-wrapper-rating-star',
      ...
      const addItemOnHorizontalOrient = (
        name: string,
        title: string,
        value: string,
      ) => {
      ...
      if (this.orient !== 'vertical') {
        let lastDescriptionEl = [...description.children][
