...
export default class TopPanel extends Component {
  ...
  renderSortingPicker(){
    ...
    const sortingPickerTitle = this.elFactory('span', {class: 'sorting-picker-title'})
    sortingPickerTitle.textContent = 'Sort'

    const selectBoxCurrentItem = this.elFactory('input', {class: 'select-box-current-item', type: 'text',
      'current-selected-item': '', placeholder: 'Select item'})

    const selectBoxBtnImg = this.elFactory('img', {class: 'select-box-open-btn-img', 
      src: './assets/images/icons/select-box-btn.svg', alt: 'select-box-btn'})    

    const selectBoxBtn = this.elFactory('button', {class: 'select-box-open-btn'}, 
      selectBoxBtnImg)
    
    const sortingPickerSelectBox = this.elFactory('div', {class: 'sorting-picker-select-box'}, 
      selectBoxCurrentItem, selectBoxBtn)
    
    const selectBoxOptionsDiv = this.elFactory('div', {class: 'select-box-items'})

    const selectBoxOptionsNotFoundItem = this.elFactory('div', {class: 'select-box-not-found'})

    const sortingFoundDiv = this.elFactory('div', {class: 'sorting-found'})
    sortingFoundDiv.textContent = 'Found: '
    const sortingFoundValue = this.elFactory('span', {class: 'sorting-found-value'})
    ...
    const addItemToOptions = (displayValue: string, value: string) => {
      const selectBoxOptionsItem = this.elFactory('div', {class: 'select-box-item', value: value})
    ...
    const removeAllSpecialSymbols = (str: string) => str.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '')
    ...
    document.body.addEventListener('click', (e) => {
      if (e.target !== selectBoxOptionsDiv && e.target !== selectBoxBtn && 
        e.target !== selectBoxBtnImg && e.target != selectBoxCurrentItem &&
        e.target !== sortingPickerSelectBox && e.target !== selectBoxOptionsNotFoundItem) {
    ...
    selectBoxCurrentItem.addEventListener('input', ()=>{
      ...
      SortingOptions.map(el => el.displayValue).map((el, i) => {
        ...
        if (selectBoxCurrentItem.value !== '' && 
          removeAllSpecialSymbols(el.toLowerCase())
          .includes(removeAllSpecialSymbols(selectBoxCurrentItem.value.toLowerCase()))) {
            addItemToOptions(SortingOptions[i].displayValue, SortingOptions[i].value)
        } 
        if (selectBoxCurrentItem.value === '' && [...selectBoxOptionsDiv.children].length !== SortingOptions.length) {
          addAllItemsToOptions()
        }
  ...
  renderChangeGrid(){
    ...
    const changeGridToSquares = this.elFactory('div', {class: 'change-grid-to-squares change-grid-item active'}, 
      this.elFactory('div', {class: 'square-grid'}), this.elFactory('div', {class: 'square-grid'}),
      this.elFactory('div', {class: 'square-grid'}), this.elFactory('div', {class: 'square-grid'}))

      const changeGridToLines = this.elFactory('div', {class: 'change-grid-to-lines change-grid-item'}, 
      this.elFactory('div', {class: 'square-grid'}), this.elFactory('div', {class: 'square-grid'}),
      this.elFactory('div', {class: 'square-grid'}), this.elFactory('div', {class: 'square-grid'}))
